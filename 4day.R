...
booleanvalue; print(booleanvalue) #소문자
...
#read line() : 표준 입력 장치로부터 입력 받는 함수
#edit() : 표준 입력 장치로부터 표형식으로 입력 받는 함수

inputData <- scan() # 빈줄이 입력될 때 까지 숫자를 입력 받는다.
class( inputData )
inputData
...
#Algorithm 요건
#1. 입력 : 반드시 0개 이상의 입력이 있어야한다.(필요있으면 입력을 받지만 무조건 적으로 받지는 않는다.)
#2. 출력 : 반드시 1개 이상의 출력이 있어야한다.(반드시 출력 1가지가 있어야한다.)
#3. 유한성 : 반드시 끝낼 수 있어야 한다.
#4. 효과성 : 효과적인 방법으로 정의되어야 한다.(현실성 필요)
#5. 명확성 : 명확한 방법으로 정의되어야 한다.



#컴퓨터 프로그램의 구조
#-순차구조 : 시작부터 끝날 때까지 차례대로 수행(위에서아래,키워드가별도로없음)
...
number <- 10
remainder <- number%%2 #if(remainder == 0){} 방법2
if (remainder == 0){
    oddeven <- '짝수'
} else {
    oddeven <- '홀수'
...
#실습 문제 : 하나의 숫자를 입력받아 짝수인지 홀수인지 출력해라

number <- as.numeric(readline('Input number:'))
if(remainder%%2){oddeven <- '짝수'
} else {
    oddeven <- '홀수'
}
cat('Number : ', number, '는', oddeven, '이다.')
...
#방법2
number <- as.numeric(readline('Input number:'))
if(number%%2==0){cat(number, '는 짝수입니다.') 
} else {
    cat(number, '는 홀수입니다.') 
}

#실숩문제 : 하나의 숫자를 입력받아 3의 배수이면 "3의 배수"
#           5의배수이면 "5의 배수:
...
#함수 형식
#
#함수 이름 <- function(인수 목록) {함수 내용에 해당하는 코드 집합}
#return(return값)
#함수 호출(호출하는것보다 정의가 더 빨리 나와야함.)
#함수명(인수 목록)
